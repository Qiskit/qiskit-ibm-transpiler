...
def test_ai_local_clifford_synthesis_wrong_backend():
...
def test_ai_local_clifford_synthesis_returns_original_circuit(fez_backend, caplog):
    ...
    ai_cliffords_synthesis_pass = PassManager(
        [
            CollectCliffords(min_block_size=2),
            AICliffordSynthesis(backend=fez_backend),
        ]
    )
...
def test_ai_local_clifford_synthesis_dont_returns_original_circuit(fez_backend, caplog):
    ...
    ai_cliffords_synthesis_pass = PassManager(
        [
            CollectCliffords(min_block_size=2),
            AICliffordSynthesis(
                backend=fez_backend, replace_only_if_better=False
            ),
        ]
    )
...
def test_ai_local_clifford_synthesis_with_backend(fez_backend):
    original_circuit = QuantumCircuit(8)
    clifford = random_clifford_from_linear_function(8)
    original_circuit.append(clifford, range(8))
    # Using decompose since we need a QuantumCircuit, not a Clifford. We created original_circuit
    # empty, so it contains only a Clifford
...
def test_ai_local_clifford_synthesis_with_coupling_map(fez_coupling_map):
    original_circuit = QuantumCircuit(8)
    clifford = random_clifford_from_linear_function(8)
    original_circuit.append(clifford, range(8))
    # Using decompose since we need a QuantumCircuit, not a Clifford. We created original_circuit
    ...
    ai_cliffords_synthesis_pass = PassManager(
        [
            CollectCliffords(min_block_size=2),
            AICliffordSynthesis(coupling_map=fez_coupling_map),
        ]
    )
